...
- You might need to install the Quarto CLI:
  - Check by running `quarto --version`
  - Install using <https://quarto.org/docs/get-started/>
...
- There are a lot of steps in a data analysis
  - Download data (`httr`) `|>` <br>
    ...
    present results (`shiny`, `rmarkdown`, `quarto`, `ggplot2`).
...
- `prereq_1`, `prereq_2`, `prereq_3`, etc are the names of the files which are used to generate `target`. These can be datasets, R scripts, etc.
  - In simple rules, `prereq_1` is the script that generates `target` and the other prerequisites are supporting datasets.
  - You can also have targets without any prerequisites. E.g., if your bash commands download a dataset, there is no prerequisite file needed.
- Each subsequent line is a *bash* command that will be evaluated in the terminal in the order listed. This sequence of commands is sometimes called a *recipe*.
  ...
  - From the terminal, it is possible to evaluate R scripts, python scripts, and render Quarto <img src="../kuato_small.png" alt="Kuato" width="3%"/> files.
...
-   Render a Quarto <img src="../kuato_small.png" alt="Kuato" width="3%"/> Document
    ...
    ```
    or, if that doesn't work, you can render a Quarto <img src="../kuato_small.png" alt="Kuato" width="3%"/> via
    ```{bash, eval = FALSE}
    Rscript -e "quarto::quarto_render('quarto_file.qmd')"
    ```
...
-   Knit an R Markdown file
    ```{bash, eval = FALSE}
    Rscript -e "rmarkdown::render('rmarkdown_file.Rmd')"
    ```
...
-   Run a SQL script using [duckdb](https://duckdb.org/docs/stable/clients/cli/overview)

    ```{bash, eval = FALSE}
    duckdb duckdb_data.duckdb < input_filet.sql
    ```
...
- tl;dr
  ...
  - For Quarto <img src="../kuato_small.png" alt="Kuato" width="3%"/> and R Markdown: Assume the working directory is the location of the qmd or Rmd file.
...
-   However, confusingly, when you render a Quarto <img src="../kuato_small.png" alt="Kuato" width="3%"/> or R Markdown file, you need to assume the working directory is the location of the Quarto <img src="../kuato_small.png" alt="Kuato" width="3%"/> or R Markdown file, not the location of `Makefile`. So in "report.qmd" you would write (**notice the double dots**)
...
- **Exercise**: What happens when you modify "panal.qmd" and you rerun `make`?
...
- **Exercise**: What is the working directory the Quarto <img src="../kuato_small.png" alt="Kuato" width="3%"/> file assumpes? What is the working directory the Makefile assumes? What is the working directory the R script assumes?
...
-   **Exercise**: Re-write the following to use automatic variables.

    ``` make
    output/report.html: analysis/report.qmd
      quarto render analysis/report.qmd
      mv analysis/report.html output/report.html
    ```
