...
- Add `make` to the path via typing the following in R:
...
- When you edit a Makefile, use the following commands instead (with the ".exe" suffix after "R" and "Rscript")

    ```{bash, eval = FALSE}
    R.exe CMD BATCH --no-save --no-restore input_file.R output_file.Rout
    ```
    
    ```{bash, eval = FALSE}
    Rscript.exe -e "rmarkdown::render('rmarkdown_file.Rmd')"
    ```
    
- Always run `make` in the Windows PowerShell.

- You can always convert the terminal back to git bash.
    
## Windows Subsystem for Linux (heavy)

- You can install Ubuntu on Windows to use all of its powerful features. This is what you would need to do for more advanced computational operations.
...
- Install `r-base-dev` using these instructions: <https://cran.r-project.org/> there naviage to "Download R for Linux > ubunut/"

    - You'll probably need to use the "Secure Apt" section as well.
...
# Comment 3
target3 : prereq3.Rmd
  Rscript -e "rmarkdown::render('prereq3.Rmd')"
```
...
- For example, suppose the file `fig_create.R` generates multiple pdf files: `fig1.pdf`, `fig2.pdf`, and `fig3.pdf`. Then we could write this rule in the two equivalent ways:
...
- If you do not like writing `R CMD BATCH --no-save --no-restore` each time, then you can save this command as a variable. The two rules are equivalent:
