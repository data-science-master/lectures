...
- For "AM" and "PM", you would use `%p` and for seconds that contain decimals, you would use `%OS`. See the help file of `parse_time()`.
...
- **Exercise**: Create a date variable from the following data frame. Then
  filter out all rows before Feb 1, 2010. If you finish early, try converting the month
  variable to the numeric representation of the month.
...
regnal |>
  mutate(start_date = str_replace(start_date, "\\(Ascension Day\\)", "27")) |>
...
Use the `start` and `end` columns to verify that the `num_years` column from Wikipedia is accurage.
