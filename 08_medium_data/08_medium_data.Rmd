...
- The **fastest** way to create and remove variables in a `data.table` is by 
  reference, where we **modify** the `data.table`, we don't create a new 
  `data.table`.
...
- To separate a column into two columns, use `tstrsplit()`.
...
There is **so** much more to data.table. It is quite literally a competitor
to the tidyverse. 

- It's own date class (a competitor to lubridate): `help(IDateTime)`
