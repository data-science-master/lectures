...
- **Exercise**: As a next step in the analysis of the `gapminder_full` data
  frame, add code in life_exp_analysis.Rmd that
    1. Loads the `gapminder_full` data frame into R, and
    2. Calculates the maximum life expectancy each year and the corresponding 
       country that had that maximum life expectancy. Hint: Use `group_by()`
       and `summarize()`.
...
- GitHub is a website that hosts git repositories. Only noobs confuse git 
  with GitHub.
...
- We are going to extend each-other's code to finalize the plot.

- Pair-up and exchange your GitHub username. Search for that person on GitHub,
  and go to their "life_exp_USERNAME" repository. The url should be something like
  "https://github.com/PARTNERNAME/life_exp_PARTNERNAME" where "PARTNERNAME" is your
  partner's username.
...
- If there is a repo already up on GitHub, you can clone a copy to your 
  local machine.

- Use `git clone` to create a local version of a repo on GitHub.

- Navigate to a directory (outside of your own git repo) where you would
  like to place your partner's git repo.
...
- **Exercise**: In your partner's file, create a scatterplot of year versus 
  maximum life expectancy. Color code by country, and add a single OLS line.
...
- **Exercise**: Save the modified file, then stage and commit the changes. 
  Finally, push the changes to your forked repo.

### Submit pull request

- A "pull request" is a request for your partner to incorporate the changes
  that you made in your forked version of their repository.

- Navigate to your forked version of your partner's repo up on GitHub. Then
  click on "New pull request".
  
    ![](./graphics/pull_request.png)\ 
    
- Write an informative title and message on what your code does,
...
- You can view all pull requests that folks have sent you from the
  "Pull requests" tab.
  
    ![](./graphics/repo8.png)\ 
    
- After you navigate to the pull request your partner sent you, you can
  see the changes that they made under the "Files changed" tab. You can
  write comments --- for example asking them to change the code before
  you accept the merge request. Or you can can just accept the merge
...
- Use `git pull` whenever there are modifications on GitHub and you want 
...
- Usually, you should only commit plain text. The reason for this is that 
  git is actually tracking **changes** in files, not the files themselves.
  This is very efficient for plain text files. But for
  non-plain text files the whole file is usally changed when you change a 
  component, so git ends up saving the whole file each each time you change it.
