...
- Consider the [Age of Empires II API](https://age-of-empires-2-api.herokuapp.com/docs).

- The documentation says that the base URL is "https://age-of-empires-2-api.herokuapp.com/api/v1/".
    ```{r}
    baseurl <- "https://age-of-empires-2-api.herokuapp.com/api/v1"
    ```

- The documentation just says that we modify this URL to obtain the different objects. E.g., to obtain a list of all the civilizations, we just add "/civilizations" at the end. We can do this with a simple `paste0()` call.

    ```{r}
    civurl <- paste0(baseurl, "/civilizations")
    ```

- We then just pass this URL to `GET()`.

    ```{r}
    civout <- GET(url = civurl)
    civout
    ```

- We would then clean this output (see rectangling below). But as a preview, we would do
    ```{r}
    tibble(civ = content(civout)[[1]]) |>
      unnest_wider(col = civ)
    ```
...
- The [OMDB API](https://www.omdbapi.com/) requires you to provide queries. The documentation says 
    ...
    But that documentation already has a query apart of it (apikey=[yourkey]).
