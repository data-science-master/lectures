...
- To obtain a free key from OMDB and access it in R:
    ...
    5. You can now always access your private key by
        ...
        ```
    6. You typically put your API key as a query parameter in a `GET()` call (see below).
...
1. You first need to "register an application" with the API provider before you can access the API. 
    - You won't actually have an app, but they will think you will!
    - This typically involves providing a name and a callback URL for your "application". 
    ...
    - You put these in `httr::oauth_app()`
    - The `appname` argument can be whatever you want.
    
    ```{r, eval = FALSE}
    my_app <- httr::oauth_app(
      appname = "personal_app_name",
      key = "client_id",
      secret = "client_secret"
    )
    ```
    
    - In the case of GitHub, register at <https://github.com/settings/developers>
    - Use any URL (like <https://github.com/>) as your homepage URL
    - Use <http://localhost:1410> as the callback url (it seems this is what you should always do).
    
    ```{r, eval = FALSE}
    git_app <- httr::oauth_app(
      appname = "gitapp",
      key = "933ffc6f53e466c58aa1",
      secret = "aa02ef46f93aa51a360f23f30f7640b445118e7f"
    )
    ```
    
    
2. You then need to set the "OAuth Endpoints". These are two URL's used for authentications. The API docs should tell you what the "authorization" URL is and what the "access" (or "token") URL is.
    - Save these using `httr::oauth_endpoint()`
    ```{r, eval = FALSE}
    my_endpts <- httr::oauth_endpoint(
      authorize = "authorize_url",
      access = "access_token_url"
    )
    ```
    
    - In the case of the GitHub API, they list their endpoints as
    ...
    ```{r, eval = FALSE}
    git_endpts <- httr::oauth_endpoint(
      authorize = "https://github.com/login/oauth/authorize",
      access = "https://github.com/login/oauth/access_token"
    )
    ```
        
3. Use the `httr::oauth2.0_token()` function to create an OAuth token using your endpoints and your app.
    
    ```{r, eval = FALSE}
    git_token <- oauth2.0_token(
      endpoint = git_endpts,
      app = git_app
    )
    ```
    
4. You then feed the token into `httr::config()`.

    ```{r, eval = FALSE}
    gtoken <- config(token = git_token)
    ```

5. You feed the output of `config()` into `GET()`, along with your API request.
...
- When using OAuth in a package, folks often (i) "cache" the token securely (because the generated token should be kept private), and (ii) ask folks to generate their own app. See the [rorcid](https://github.com/ropensci-archive/rorcid/blob/afa74daabf18499841e843a755b182c73ab545ea/R/auth.R) example of authorization here.
