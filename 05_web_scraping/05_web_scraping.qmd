...
-   If you want all of the `b`s that are within a `p`, you can use ` `.
    ```{r}
    html_k |>
      html_elements("p b") |>
      html_text()
...
-   Use `read_html()` to save an HTML file to a variable. The variable will
  be an "`xml_document`" object
    ...
    #| eval: true
    #| echo: false
    html_obj <- read_html("./imdb_100_3.html")
    ```
...
- [XML](https://en.wikipedia.org/wiki/XML) stands for 
  ...
  (like HTML and Markdown), useful for representing a document.
...
#| echo: false
#| eval: true
mosque <- read_html("mosque_2.html")
```
